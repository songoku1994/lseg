...
class depthwise_clipseg_conv(mindspore.nn.Cell):
    def __init__(self):
        super(depthwise_clipseg_conv, self).__init__()
        # self.depthwise = nn.Conv2d(1, 1, kernel_size=3, padding=1)
        self.depthwise = mindspore.nn.Conv2d(1, 1, kernel_size=3, padding=1)
    
    def depthwise_clipseg(self, x, channels):
        # x = torch.cat([self.depthwise(x[:, i].unsqueeze(1)) for i in range(channels)], dim=1)
        x = mindspore.ops.Concat([self.depthwise(x[:, i].unsqueeze(1)) for i in range(channels)], dim=1)
        return x
...
class depthwise_conv(mindspore.nn.Cell):
    def __init__(self, kernel_size=3, stride=1, padding=1):
        super(depthwise_conv, self).__init__()
        # self.depthwise = nn.Conv2d(1, 1, kernel_size=kernel_size, stride=stride, padding=padding)
        self.depthwise = mindspore.nn.Conv2d(1, 1, kernel_size=kernel_size, stride=stride, padding=padding)
...
class depthwise_block(mindspore.nn.Cell):
    def __init__(self, kernel_size=3, stride=1, padding=1, activation='relu'):
        super(depthwise_block, self).__init__()
        self.depthwise = depthwise_conv(kernel_size=3, stride=1, padding=1)
        if activation == 'relu':
            # self.activation = nn.ReLU()
            self.activation = mindspore.nn.ReLU()
        elif activation == 'lrelu':
            # self.activation = nn.LeakyReLU()
            self.activation = mindspore.nn.LeakyReLU()
        elif activation == 'tanh':
            # self.activation = nn.Tanh()
            self.activation = mindspore.nn.Tanh()
...
class bottleneck_block(mindspore.nn.Cell):
    def __init__(self, kernel_size=3, stride=1, padding=1, activation='relu'):
...
class BaseModel(mindspore.nn.Cell):
    def load(self, path):
        ...
        parameters = mindspore.load_checkpoint(path, map_location=torch.device("cpu"))
...
def _make_fusion_block(features, use_bn):
    ...
    return FeatureFusionBlock_custom(
        features,
        activation=mindspore.nn.ReLU(False),
        deconv=False,
        bn=use_bn,
        expand=False,
        align_corners=True,
...
class LSeg(BaseModel):
    def __init__(
        ...
        **kwargs,
        ...
        self.logit_scale = mindspore.Parameter(mindspore.ops.Ones([]) * np.log(1 / 0.07)).exp()
        if backbone in ["clipRN50x16_vitl16_384"]:
            self.out_c = 768
        else:
            self.out_c = 512
        # self.scratch.head1 = nn.Conv2d(features, self.out_c, kernel_size=1)
        self.scratch.head1 = mindspore.nn.Conv2d(features, self.out_c, kernel_size=1)
